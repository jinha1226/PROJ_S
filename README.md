...
사용합니다. 전투 모드 없이 적을 눌러 공격하며, 동료·적은 각자의 행동 속도에
따라 플레이어 행동 사이에 움직입니다. 아래의 과거 조우·배치 설명은 기존 코어
검증 장면에 해당합니다.

Godot 4.6/GDScript로 작성한 헤드리스 우선 결정론적 시뮬레이션 코어입니다. 현재 범위는 턴과 세계시간의 분리, 결정론적 예약 처리, 행동 preview/timeline, 지형·8방향 MOVE·점유, 원소 노출, 종족별 위험 평가, 공통 피해와 격리된 신체 상태 B0 모델, 저장·복원, **종족 관계 기준선이 개인 경험보다 우선하는 관계**, 동일-grid 4인 파티 조우, 연속형 HEXACO 동료 AI, 적 무리 전술, 사기·공포 전염, 파티 공유 인지와 DCSS형 예외 명령, 화면 밖 축약 전투의 P4-1 forecast와 P4-2a 독립 권위 리듀서입니다. 범용 기억·소문, 다지역 몬스터 생태, 축약 전투의 제품 `WorldState` 연결·상세 복귀와 최종 대형 월드는 아직 구현하지 않았습니다.

## 아트 파일 찾기

- [현재 게임 에셋](assets/README.md)
- [원본·채택한 참고 이미지·이전 시안](art/README.md)
- [실제 적용 화면과 검증 기록](docs/art/README.md)
...
메인 파티 맵은 ASCII 재질형 2D 탑뷰가 기본입니다. 단일 중립 배경 위에 문자만 그리며 바닥은 `.`, 벽은 `#`이고 인접한 벽 중심 사이에도 표시용 `#`를 넣어 촘촘하게 연결합니다. 이 추가 문자는 충돌·클릭 좌표를 바꾸지 않습니다. 주인공은 `@`, 인물은 종족별 `h/g/k/d/e/o/b/a/s`, 장비는 `/ | 7 ! ^ ) + [ ]`로 읽힙니다. 물 `~`·풀 `"`·불 `^`·독 `%`만 좌표별로 어긋난 낮은 주기의 동작을 가집니다. 기억/미관측 셀에는 동작이나 정보 누출이 없고 와이드 줌에서는 환경 동작이 고정됩니다. 근접 공격은 캐릭터 전체 범프와 대상 셀 VFX·히트스톱·화면 흔들림으로 표현합니다. 활/쇠뇌는 `[사격]`, 쇠뇌 장전은 전투 중 `[RELOAD]`로 조작합니다. 전체 규칙은 [ASCII 재질형 2D 탑뷰 시각 언어](docs/ASCII_MATERIAL_VISUAL_LANGUAGE.ko.md)에 정리되어 있습니다.
