...
const TEST_FILE := "test_product_tab_attack.gd"

func _init()->void:
	var script=load("res://tests/"+TEST_FILE);var total:=0;var failed:=0
	if script==null or not script.can_instantiate():
		printerr("FAIL product Tab attack test script failed to load");quit(1);return
	...
	print("---- Product Tab attack: %d tests, %d failed ----"%[total,failed])
