...
func _init()->void:
	...
	assert(Catalog.DATA.definitions.size()==19)
	for row in Catalog.DATA.definitions:
		...
		for death in range(1,201):
			var rules:String=Drops.ELEMENTAL_RULESET_ID if str(row.essence_id).begins_with("PART_") else Drops.RULESET_ID
			var result:=Drops.rolls_for(44,death,row.species_id,rules)
			assert(result==Drops.rolls_for(44,death,row.species_id,rules),"stable roll")
			for roll in result:
	...
	print("MONSTER ABILITY DROPS: PASS (19 mappings)")
	quit()
