...
func run() -> void:
	...
	for viewport in [Vector2i(390,844),Vector2i(412,915)]:
		...
		check(chip != null and chip.text.contains("유물 찾기") and scene.get_global_rect().encloses(chip.get_global_rect()),"objective chip visible and on screen")
		...
		check("원정" in nav_texts and "전술" not in nav_texts,"solo footer swaps tactics for the expedition menu")
