...
func run()->void:
	...
	check(ui.member_detail_title.get_theme_font("font")==preload("res://playtest/dark_pixel_ui_skin.gd").PixelFont,"status pixel font")
	...
	check(ui.member_status_window.find_child("StatusBodyState",true,false)!=null,"body state section preserved")
	...
	check(ui.member_item_equipment_grid.get_child_count()==9,"seven slots and two spacers")
	await shot("/tmp/character-equipment.png")
	var grid=ui.member_item_equipment_grid
	var portrait=ui.member_item_window.find_child("EquipmentPortrait",true,false)
	check(portrait.get_global_rect().end.x<=grid.global_position.x,"portrait left of equipment")
	check(grid.size.x<=164,"equipment grid stays compact")
	for slot in ["HEAD","FEET"]:
		check(grid.get_node("EquipmentSlot"+slot).disabled,"unimplemented slots cannot send equip commands")
	check(grid.get_node("EquipmentSlotHEAD").position.x==grid.get_node("EquipmentSlotARMOR").position.x,"head above body")
	check(grid.get_node("EquipmentSlotACCESSORY_1").position.x==grid.get_node("EquipmentSlotOFF_HAND").position.x,"ring below left hand")
