...
const TEST_FILE := "test_party_auto_flow.gd"
...
func _init() -> void:
	...
	for method in probe.get_method_list():
		...
		elif completed==false and test_case.errors.is_empty():test_case.errors.append("test returned false without a recorded assertion")
	...
	print("---- Party auto flow: %d tests, %d failed ----"%[total,failed])
