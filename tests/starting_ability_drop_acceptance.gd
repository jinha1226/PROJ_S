...
func run()->void:
	...
	check(not found,"prototype firebolt essence no longer starts underfoot")
	var attack_before:int=session._member_combat_stats(hero).attack_power
	check(world.party_encounter.member(hero).active_skill_ids().is_empty(),"new hero has no free test skills")
	...
	check(restored.load_session_json(saved).accepted and restored.sim.snapshot()==session.sim.snapshot(),"clean start and equipment replay exactly")
