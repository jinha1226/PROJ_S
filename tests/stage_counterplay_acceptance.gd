...
func run():
	...
	for i in range(5):check(Stage.finish_round(s.sim),"predeadline counter")
	check(Stage.current(w).turn==6 and w.party_encounter.enemy_ids.size()==count,"no wave through six rounds")
	check(Stage.finish_round(s.sim),"deadline reinforcement")
	check(w.party_encounter.enemy_ids.size()==count+2,"two enemies at seven rounds")
