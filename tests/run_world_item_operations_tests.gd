...
const TEST_FILE := "test_world_item_operations.gd"
...
func _init() -> void:
	...
	if script == null or not script.can_instantiate():
		printerr("FAIL world item operations test script failed to load")
		quit(1)
		return
	...
	print("---- World item operations: %d tests, %d failed ----" % [total, failed])
