...
const TEST_FILE := "test_phase5_combat_status_lifecycle.gd"
...
func _init() -> void:
	...
	for method in probe.get_method_list():
		...
		if typeof(completed)!=TYPE_BOOL:test_case.errors.append("test did not return explicit true completion")
		elif completed==false and test_case.errors.is_empty():test_case.errors.append("test returned false without assertion")
	...
	print("---- Phase 5 checkpoint A: %d tests, %d failed ----"%[total,failed]);quit(1 if failed>0 else 0)
