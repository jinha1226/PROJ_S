...
func recorded() -> void:
	...
	check(int(d.s.parts_bag.get("RAT_GNAW/"+str(d.foe.get("part_kind","")),0)) == 1,"the selected physical part drops")
...
func unchanged_drop() -> void:
	...
	Gear.roll_part(d.s,d.foe)
	var dropped: bool = int(d.s.parts_bag.get("RAT_GNAW/"+str(d.foe.get("part_kind","")),0)) == 1
