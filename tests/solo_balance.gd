...
func play(s) -> Dictionary:
	...
	while s.phase == "BATTLE" and actions < 1500:
		...
		actions += 1
	return {"reason":s.result.get("reason","STUCK"),"actions":actions,"heals":heals,"hp":hero.hp,"food":s.food,"light":s.light,"bank":s.bank,"stress":hero.stress,"calming":2-s.supplies[1]}

func run() -> void:
	var wins := 0
	for seed_value in range(SEEDS):
		var row := play(Session.new(seed_value,true,false,true))
		print("seed %d: %s · 행동 %d · 회복 %d · 체력 %d · 식량 %d · 밝기 %d · 자금 %d" % [seed_value,row.reason,row.actions,row.heals,row.hp,row.food,row.light,row.bank])
