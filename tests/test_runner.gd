...
const RunnerCombatSuite = preload("res://tests/test_runner_combat.gd")
...
func _init() -> void:
	...
	var runner_combat_suite = RunnerCombatSuite.new()
	failures.append_array(runner_combat_suite.run())

	if failures.is_empty():
		print("TESTS PASSED (legacy systems, duel prototypes, mocap, and runner combat)")
		quit(0)
		return
