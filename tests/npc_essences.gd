...
func run() -> void:
	...
	check(blunt.equipped_abilities == [Essences.canonical("ORC_CLEAVER")],"low agreeableness wears the berserker")
	var careful: Dictionary = stranger(s,1,{"A":900,"C":950,"O":100},pool,1)
	NpcEssences.choose(s,careful)
	check(careful.equipped_abilities == [Essences.canonical("HOB_TAUNT")],"high conscientiousness wears the guard")
	...
	var dropped: String = Essences.base_of(str(foe.part_id))+"/"+str(foe.part_kind)
	check(Essences.absorbed(hunter,dropped),"the first kill of a species gives the npc its essence")
	check(hunter.equipped_abilities[0] == dropped,"and it wears it at once")
	...
	check(second.hp <= 0 and Essences.absorbed(other,Essences.base_of(str(second.part_id))+"/"+str(second.part_kind)),"an npc's own kill in play gives it the essence")
