...
func session_layer() -> void:
	...
	for id in Session.Essences.catalog():
		check(s.parts_bag.get(Essences.canonical(str(id)),0) >= 1,"every catalog part is in the bag: "+id)
...
func scene_layer() -> void:
	...
	for frame in range(4): await process_frame
	for id in Session.Essences.catalog():
		check(s.parts_bag.get(Essences.canonical(str(id)),0) >= 1,"fixture grants "+id)
