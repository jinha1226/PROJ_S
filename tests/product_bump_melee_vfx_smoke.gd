...
func _run()->void:
	...
	_check(bool(hit_case.get("hit",false)),"seed 44 product bump owns the HIT feedback")
	_check(bool(miss_case.get("miss",false)),"seed 4/wait 1 product bump owns the MISS feedback")
	for row in [hit_case,miss_case]:
		...
		_check(bool(row.get("enemy_bump",false)),
			"%s same-turn enemy counterattack owns an independent bump"%row.get("label","case"))
...
func _exercise_product_bump(world_seed:int,exploration_waits:int)->Dictionary:
	...
	var hero_effect:Dictionary={};var enemy_bump:=false
	for effect in sandbox.grid.melee_vfx.active_effects():
		if Vector2i(effect.attacker_grid_pos)==hero_position:hero_effect=effect
		elif Vector2i(effect.attacker_grid_pos)==hero_position+Vector2i.RIGHT:enemy_bump=true
	var drawn:=false
	if not hero_effect.is_empty():
		...
		var actor_spec:Dictionary=sandbox.grid.actor_glyph_draw_spec(hero,
			int(hero_effect.started_at_ms)+int(params.contact_at_ms)+12)
		var settled:Dictionary=sandbox.grid.actor_glyph_draw_spec(hero,
			int(hero_effect.started_at_ms)+int(params.bump_duration_ms))
		var travel:=Vector2(actor_spec.center).distance_to(Vector2(settled.center))
		drawn=bool(actor_spec.get("bump_motion",{}).get("active",false)) \
			and not bool(actor_spec.get("weapon_swing",{}).get("active",false)) \
			and travel>=2.0 and int(hero_effect.get("rendered_frames",0))>=1
	var result:={"label":"HIT" if world_seed==44 else "MISS",
		...
		"drawn":drawn,"enemy_bump":enemy_bump,
