...
const TEST_FILE:="test_dark_fantasy_ascii_ui.gd"
...
func _init()->void:
	...
	for method in script.new().get_method_list():
		...
		if typeof(completed)!=TYPE_BOOL:test_case.errors.append("test did not return bool")
		elif not completed and test_case.errors.is_empty():test_case.errors.append("false without assertion")
	...
	print("---- Dark fantasy ASCII UI: %d tests, %d failed ----"%[total,failed])
