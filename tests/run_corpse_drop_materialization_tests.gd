...
const TEST_FILE := "test_corpse_drop_materialization.gd"
...
func _init() -> void:
	...
	if script == null or not script.can_instantiate():
		printerr("FAIL corpse drop test script failed to load")
		quit(1)
		return
	...
	print("---- Corpse drop materialization: %d tests, %d failed ----" % [total, failed])
