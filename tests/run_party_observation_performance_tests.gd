...
const TEST_FILE := "test_party_observation_performance.gd"
...
func _init()->void:
	...
	for method in script.new().get_method_list():
		...
		if typeof(completed)!=TYPE_BOOL:
			...
			test_case.errors.append("test returned false without assertion")
	...
	print("---- Party observation performance: %d tests, %d failed ----"%[total,failed])
