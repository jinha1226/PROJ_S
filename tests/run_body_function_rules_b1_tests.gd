...
const TEST_FILE:="test_body_function_rules_b1.gd"


func _init()->void:
	var script=load("res://tests/"+TEST_FILE);var total:=0;var failed:=0
	if script==null or not script.can_instantiate():
		printerr("FAIL body function B1 test script failed to load");quit(1);return
	...
	print("---- Body function B1: %d tests, %d failed ----"%[total,failed])
