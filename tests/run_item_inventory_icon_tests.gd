...
const TEST_FILE:="test_item_inventory_icons.gd"

func _init()->void:
	var script=load("res://tests/"+TEST_FILE);var total:=0;var failed:=0
	if script==null or not script.can_instantiate():
		printerr("FAIL item inventory icon test script failed to load");quit(1);return
	...
	print("---- Item inventory icons: %d tests, %d failed ----"%[total,failed])
