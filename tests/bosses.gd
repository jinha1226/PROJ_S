...
func eater() -> void:
	...
	BossAI.turn(s,boss)
	check(hero.get("sealed",{}).has(Essences.canonical("ORC_CLEAVER")),"the first slotted stone is sealed (no tiers to rank)")
	check(Essences.canonical("ORC_CLEAVER") not in Essences.equipped(hero) and StatSheet.value(s,hero,"atk") < attack,"a sealed essence leaves the stat sheet")
	check(not Abilities.holds(hero,"ORC_CLEAVER"),"and its technique")
	for turn in range(3): BossAI.turn(s,boss)
	check(not hero.get("sealed",{}).has(Essences.canonical("ORC_CLEAVER")) and Essences.canonical("ORC_CLEAVER") in Essences.equipped(hero),"three turns later it is free")
