...
const TEST_FILE := "test_protagonist_equipment_defense_v3.gd"
...
func _init() -> void:
	...
	if script == null or not script.can_instantiate():
		printerr("FAIL defense v3 test script failed to load")
		quit(1)
		return
	...
	print("---- Protagonist equipment defense v3: %d tests, %d failed ----" % [total, failed])
