...
func _check_viewport(viewport_size:Vector2)->void:
	...
	_check(sandbox.grid.visible_cell_count==Session.PRODUCT_ZOOM_DEFAULT_CELL_COUNT,
		"%s product camera did not use the shared %d-cell default"%[viewport_size,Session.PRODUCT_ZOOM_DEFAULT_CELL_COUNT])
	_check(sandbox.phase_panel.visible \
		...
		and sandbox.expedition_floor_label.text=="지하 1층" \
		and sandbox.return_timer_label.text.begins_with("귀환까지 ") \
		and sandbox.find_child("Ascii3DLabButton",true,false)==null,
		"%s product top rail lacks minimap, floor/return timer, or main menu"%viewport_size)
	_check(_inside_rect(sandbox.phase_panel,sandbox.minimap_frame) \
		and _inside_rect(sandbox.phase_panel,sandbox.expedition_floor_label) \
		and _inside_rect(sandbox.phase_panel,sandbox.return_timer_label) \
		and _inside_rect(sandbox.phase_panel,sandbox.product_menu_button) \
		and sandbox.minimap_frame.get_global_rect().end.x<=sandbox.expedition_floor_label.get_global_rect().position.x+0.5 \
...
func _check_active_route_direction_override(viewport_size:Vector2)->void:
	...
	if chosen!=Vector2i.ZERO:
		...
		_check(int(session.party_status().step_index)==step_before+1 \
			...
			"%s active route → map touch did not cancel then commit exactly once"%viewport_size)
		await process_frame;await process_frame
		_check(int(session.party_status().step_index)==step_before+1,
...
func _check_product_direction_touch(viewport_size:Vector2)->void:
	# Movement is a map touch. Each adjacent cell tap is exactly one authoritative
	# step, which is the contract the removed D-pad used to carry.
	var session=_baseline_solo_session()
	...
	root.add_child(sandbox);await process_frame;await process_frame
	_check(sandbox.find_child("ProductDirectionPad",true,false)==null \
		and sandbox.find_child("ProductWaitCenter",true,false)==null,
		"%s D-pad survived beside map touch movement"%viewport_size)
	var steps:Array=[Vector2i(-1,-1),Vector2i(0,-1),Vector2i(1,-1),Vector2i(-1,0),
		Vector2i(1,0),Vector2i(-1,1),Vector2i(0,1),Vector2i(1,1)]
	for row_index in range(steps.size()):
		var direction:Vector2i=steps[row_index]
		if row_index>0:
		...
		await _screen_touch_grid_cell(sandbox,before+direction,31+row_index)
		...
		_check(after==before+direction and int(after_status.step_index)==step_before+1 \
			and session.command_journal.size()==journal_before+1,
			"%s map touch %s was not exactly one authoritative step: %s/%s step %d/%d journal %d/%d phase %s/%s"%[
				viewport_size,direction,before,after,step_before,int(after_status.step_index),journal_before,
				session.command_journal.size(),before_status.safe_phase,after_status.safe_phase])
		await process_frame;await process_frame
		_check(int(session.party_status().step_index)==step_before+1 \
			and session.command_journal.size()==journal_before+1,
			"%s map touch %s double-committed on deferred refresh"%[viewport_size,direction])
	session=_baseline_solo_session()
	sandbox._reset_run_ui_transients();sandbox.initialize_for_headless_test(session,false)
	await process_frame;await process_frame
	...
	await _screen_touch_button(sandbox,sandbox.product_wait_guard_button,32)
	...
	_check(wait_after.protagonist_position==wait_position \
		...
		"%s dock WAIT was not exactly one stationary authoritative turn"%viewport_size)
	await process_frame;await process_frame
	_check(int(session.party_status().step_index)==int(wait_before.step_index)+1,
		"%s dock WAIT double-committed"%viewport_size)
	...
	await _screen_touch_grid_cell(sandbox,contact_origin+Vector2i.UP,41)
	...
	_check(Vector2i(int(contact_after.protagonist_position[0]),
		...
		("%s first monster-facing map touch jumped cells or lost its facing: " \
	...
	await _screen_touch_grid_cell(sandbox,contact_origin,42)
...
func _check_direct_solo_combat_log(viewport_size:Vector2)->void:
	...
	_check(int(session.party_status().step_index)==combat_step_before+1,
		"%s adjacent enemy map touch did not commit exactly one combat turn"%viewport_size)
