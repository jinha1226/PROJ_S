...
const TEST_FILE := "test_world_occupancy_index.gd"
...
func _init() -> void:
	...
	if script == null or not script.can_instantiate():
		printerr("FAIL occupancy index test script failed to load")
		quit(1)
		return
	...
	print("---- Occupancy index: %d tests, %d failed ----" % [total, failed])
