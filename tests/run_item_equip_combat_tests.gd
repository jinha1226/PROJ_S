...
const TEST_FILE:="test_item_equip_during_combat.gd"

func _init()->void:
	var script=load("res://tests/"+TEST_FILE);var total:=0;var failed:=0
	if script==null or not script.can_instantiate():
		printerr("FAIL item equip combat test script failed to load");quit(1);return
	...
	print("---- Item equip in combat: %d tests, %d failed ----"%[total,failed])
