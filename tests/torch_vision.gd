...
func _initialize() -> void:
	...
	check(s.floor_state.visible.size() == 149 and s.floor_state.visible.has(Vector2i(57,50)) and not s.floor_state.visible.has(Vector2i(57,57)),"full torch reveals wider radius-seven circle, not square corners")
	var previous := 150
	...
	check(s.floor_state.explored.size() == 149,"darkness preserves discovered terrain memory")
	...
	s.light = 5; s.round_number = 19; s.floor_state.observe(s)
	check(s.floor_state.visible.size() == 21,"pre-decay threshold")
	s.act("WAIT",s.party[0].pos)
	check(s.light == 3 and s.floor_state.visible.size() == 13,"light decay updates visibility in the same turn")
