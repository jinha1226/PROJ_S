...
const TEST_FILE := "test_opening_fixed_event.gd"
...
func _init() -> void:
	...
	if script == null or not script.can_instantiate():
		printerr("FAIL opening fixed event test script failed to load")
		quit(1)
		return
	...
	print("---- Opening fixed event: %d tests, %d failed ----" % [total, failed])
