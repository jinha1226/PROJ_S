...
const TEST_FILE := "test_item_reward_progression.gd"
...
func _init() -> void:
	...
	if script == null or not script.can_instantiate():
		printerr("FAIL item reward progression test script failed to load")
		quit(1)
		return
	...
	print("---- Item reward progression: %d tests, %d failed ----" % [total, failed])
