...
func _init()->void:
	var script=load("res://tests/test_party_exploration_patrol.gd")
	if script==null or not script.can_instantiate():
		printerr("FAIL patrol test script failed to load");quit(1);return
	...
	for method in script.new().get_method_list():
		...
		if test_case.errors.is_empty():print("PASS patrol :: %s"%method.name)
		else:
			failed+=1
			for error in test_case.errors:print("FAIL patrol :: %s -- %s"%[method.name,error])
	print("---- Patrol: %d tests, %d failed ----"%[total,failed])
