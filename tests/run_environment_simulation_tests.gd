...
const TEST_FILE := "test_environment_simulation.gd"
...
func _init() -> void:
	...
	if script == null or not script.can_instantiate():
		printerr("FAIL environment simulation test script failed to load")
		quit(1)
		return
	...
	print("---- Environment simulation: %d tests, %d failed ----" % [total, failed])
