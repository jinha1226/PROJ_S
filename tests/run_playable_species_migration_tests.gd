...
const TEST_FILE := "test_playable_species_migration.gd"
...
func _init() -> void:
	...
	if script == null or not script.can_instantiate():
		printerr("FAIL playable species migration test script failed to load")
		quit(1)
		return
	...
	print("---- Playable species migration: %d tests, %d failed ----" % [total, failed])
