...
func run()->void:
	...
	for turn in range(30):
		...
		var result=Field.step(session.sim,Action.new("CRISIS",world.party_control_actor_id()) if not world.can_act(world.party_control_actor_id(),world.world_time) or party.member(world.party_control_actor_id()).busy_until>world.world_time else Action.hold(world.party_control_actor_id()))
		...
		for event in result.events:
			if event.type in ["entity.downed","entity.died"] and event.target_id in party.active_party_member_ids:
				saw_loss=true
				if event.type=="entity.downed":check(int(event.data.downed_resolve_at)==event.world_time+1000,"downed deadline is ten full turns")
				if event.type=="entity.died":
					var pressure=world.event_by_id(event.cause_id)
					var downed=world.event_by_id(pressure.cause_id)
					check(event.world_time==downed.world_time+1000,"death only at exact deadline")
		if party.safe_phase=="PARTY_DEFEATED":break
	check(saw_loss,"fixture exercises party incapacitation")
	check(world.combatant_states[party.protagonist_id].life_state=="DEAD" and party.safe_phase=="PARTY_DEFEATED","only actual protagonist death ends run")
	print("PARTY RESCUE LIFECYCLE: ","PASS" if failures.is_empty() else "FAIL", " ",failures)
