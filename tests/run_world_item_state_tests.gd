...
const TEST_FILE:="test_world_item_state.gd"

func _init()->void:
	var script=load("res://tests/"+TEST_FILE);var total:=0;var failed:=0
	if script==null or not script.can_instantiate():
		printerr("FAIL world item state test script failed to load");quit(1);return
	...
	print("---- World item state: %d tests, %d failed ----"%[total,failed])
