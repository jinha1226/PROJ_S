...
func run() -> void:
	...
	for viewport in [Vector2i(390,844),Vector2i(430,844),Vector2i(412,915)]:
		...
		for member in range(2):
			for tab in ["상태","성격","기억","이능"]:
				...
				if tab == "이능": check(scene.modal_content.find_child("EssenceSlots",true,false).get_child_count() == 10,"ten essence slots")
				...
				if "--capture" in OS.get_cmdline_user_args() and viewport == Vector2i(390,844) and member == 1:
					...
					root.get_texture().get_image().save_png("/tmp/folio-"+str(["상태","성격","기억","이능"].find(tab))+".png")
	...
	scene.session.party[1].essences = {"PUSH":1,"GUARD":1}
	...
	check(scene.session.parts_bag.get("PUSH",0) == 1 and scene.session.party[1].essences.has("PUSH"),"the replaced essence stays absorbed")
	...
	scene.session.phase = "CAMP"
	check(scene.session.unequip_part(1,0) and scene.session.parts_bag.BOMB == 0 and scene.session.party[1].essences.has("BOMB"),"unequipping keeps the essence absorbed")
	scene.show_character(1,"이능")
	for frame in range(3): await process_frame
	EssenceTab.chooser(scene,0)
	for frame in range(3): await process_frame
	var picks: Array = scene.item_detail.find_children("*","Button",true,false).filter(func(b): return b.name == "EssencePick_BOMB")
