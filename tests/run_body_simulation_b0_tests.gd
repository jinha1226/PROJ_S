...
const TEST_FILE := "test_body_simulation_b0.gd"


func _init() -> void:
	var script=load("res://tests/"+TEST_FILE);var total:=0;var failed:=0
	if script==null or not script.can_instantiate():
		printerr("FAIL body simulation B0 test script failed to load");quit(1);return
	...
	print("---- Body simulation B0: %d tests, %d failed ----"%[total,failed])
