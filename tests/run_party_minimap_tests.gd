...
const TEST_FILE:="test_party_minimap.gd"
...
func _init()->void:
	...
	for method in script.new().get_method_list():
		...
		elif completed==false and test_case.errors.is_empty():test_case.errors.append("false without assertion")
	...
	print("---- Party vector minimap: %d tests, %d failed ----"%[total,failed])
