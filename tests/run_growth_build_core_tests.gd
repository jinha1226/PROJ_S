...
const TEST_FILE := "test_growth_build_core.gd"
...
func _init() -> void:
	...
	if script == null or not script.can_instantiate():
		printerr("FAIL growth build core test script failed to load")
		quit(1)
		return
	...
	print("---- Growth build core: %d tests, %d failed ----" % [total, failed])
