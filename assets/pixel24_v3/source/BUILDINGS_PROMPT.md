# Base building atlas — source prompt (generated 2026-09-09)
...
Integration notes: exterior wall bands are 3–4 native pixels thick, NOT one
24px tile thick. Footprints and collision remain defined by gameplay rules.
Generated crops are explicitly measured and fitted to 72x72 or 72x48 in the
asset pipeline; art does not change building size, paths, entrances, or levels.
Keep selection, construction progress, and interaction overlays code-drawn.
...
Each cell is a 3x3-tile plot; a building either fills the whole 3x3 plot or exactly the TOP 3x2 tiles of it with the bottom tile row left fully transparent. Structures snap to tile edges: outer wall bands are only 3–4 logical pixels thick on a 72px-wide building, never a whole 24px tile thick; corners square, generous interior space. Crisp coarse square pixel clusters like enlarged native 24px tiles, 1 logical-pixel charcoal contour, 2-3 flat tones per material, at most 12 colors per building, no gradients, no antialiasing, no drop shadows, no glow.
...
Built-in image generation, style references: `../review/terrain_floor1-6x.png`
and `../review/nearest-contact-6x.png`. Selected source:
`buildings-generated-v2.png`, using a magenta key background. Source cells are
approximate illustrations rather than exact native tile geometry; runtime crops
and dimensions are normalized separately, without changing gameplay footprints.

Perspective/simplification correction applied to the retained v1 source:

> Use case: precise-object-edit. Edit target: this 6 columns by 3 rows top-down building sprite atlas. Preserve all eighteen building subjects, their order, material progression (camp, timber, stone), muted palette, magenta RGB(255,0,255) background, exact grid positions and outer bounding boxes. Correct ONLY the perspective and overbusy texture: use absolutely vertical overhead camera, no visible vertical wall faces. Walls are narrow FLAT plan-view ribbons 3-4 logical pixels thick on a 72px wide building, with a light rim. Remove the tall dark back-wall facade, horizontal stacked plank facade and stone facade; replace them with floor-visible space inside a narrow wall-top band. Objects are seen from above: barrel circular lids, bed rectangular blanket tops, crates square lids, counters horizontal tops. Gate: two square post/tower TOPS connected by a thin wall-top band with a clearly open passage, no frontal gate facade, no vertical pointed pickets. Reduce random pebble/noisy floor texture substantially, keep just broad muted floor tones and the main identifying props. Crisp coarse pixel clusters and dark single-pixel outline remain. Do not add text, labels, grid lines, cast shadows, gradients, extra props or roofs. Preserve separate isolated sprites and pure magenta between them.
