...
func canonical_manifest()->Array:
	var rows:Array=[]
	for action_id in action_ids():
		var definition=_definitions[action_id];var legal_rows:Array=[];var score_rows:Array=[]
		...
		rows.append({"action_id":str(action_id),"atomic_verb":str(definition.atomic_verb),
			...
			"score_terms":score_rows})
	return rows.duplicate(true)

func ruleset_fingerprint()->String:
	return JSON.stringify(canonical_manifest()).sha256_text()

func evaluate_actor(actor,other,inputs:Dictionary,seed:int,turn_index:int)->Dictionary:
	...
	for action_id in action_ids():
		...
		for term in definition.score_terms:
			...
			total+=contribution
		var jitter:=HexacoScript.sample(seed,actor.entity_id,"duel/"+str(turn_index)+"/"+action_id,31)-15
		total+=jitter
