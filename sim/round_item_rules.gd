...
static func assess(w,id:int,operation:Dictionary)->Dictionary:
	...
	if id!=w.party_encounter.protagonist_id:return {"accepted":false,"reason":"round_item_actor"}
	var pos:Vector2i=w.entities[id].position
	match operation.action:
		"USE":return Ops.preview_use(w,id,operation.instance_id)
		"PICKUP":return Ops.preview_pickup(w,id,operation.instance_id,pos)
		"EQUIP":return Ops.preview_equip(w,id,operation.instance_id,operation.slot)
		"UNEQUIP":return Ops.preview_unequip(w,id,operation.slot)
		"DROP":return Ops.preview_drop(w,id,operation.instance_id,pos)
		"DISCARD":return Ops.preview_discard(w,id,operation.instance_id)
...
static func commit(sim,id:int,operation:Dictionary)->Dictionary:
	...
	match operation.action:
		...
		"USE":
			...
			if Mystery.has(definition_id):return preload("res://playtest/mystery_item_service.gd").use(Host.new(sim),operation.instance_id,operation.selection)
