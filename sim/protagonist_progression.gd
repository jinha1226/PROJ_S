class_name ProtagonistProgression
extends RefCounted

const RegistryScript=preload("res://sim/progression_registry.gd")
const SCHEMA_VERSION:=1

var schema_version:=SCHEMA_VERSION
var xp_total:=0
var training_focus:Dictionary=RegistryScript.DEFAULT_FOCUS.duplicate(true)
var skill_training:Dictionary={"MELEE":0,"GUARD":0,"EXPLORATION":0}
var processed_victory_event_ids:Array[int]=[]

func award_victory(event_id:int)->bool:
	if event_id<=0 or event_id in processed_victory_event_ids:return false
	if xp_total>RegistryScript.MAX_XP-RegistryScript.VICTORY_XP:return false
	if not RegistryScript.focus_error(training_focus).is_empty():return false
	xp_total+=RegistryScript.VICTORY_XP
	for skill_id in RegistryScript.SKILL_IDS:
		skill_training[skill_id]=int(skill_training[skill_id]) \
			+int(RegistryScript.VICTORY_XP*int(training_focus[skill_id])/RegistryScript.FOCUS_TOTAL)
	processed_victory_event_ids.append(event_id)
	return true

func set_focus_preset(skill_id:String)->bool:
	var preset:=RegistryScript.focus_preset(skill_id)
	if preset.is_empty() or preset==training_focus:return false
	training_focus=preset.duplicate(true)
	return true

func rank(skill_id:String)->int:
	return RegistryScript.skill_rank(int(skill_training.get(skill_id,0)))

func to_dict()->Dictionary:
	var focus_rows:Array=[];var training_rows:Array=[]
	for skill_id in RegistryScript.SKILL_IDS:
		focus_rows.append({"skill_id":skill_id,"weight":int(training_focus[skill_id])})
		training_rows.append({"skill_id":skill_id,"training_total":int(skill_training[skill_id])})
	return {"schema_version":schema_version,"xp_total":xp_total,
		"training_focus":focus_rows,"skill_training":training_rows,
		"processed_victory_event_ids":processed_victory_event_ids.map(func(id):return str(id))}

static func from_dict(row:Dictionary):
	var value=load("res://sim/protagonist_progression.gd").new()
	value.schema_version=SCHEMA_VERSION;value.xp_total=int(row.xp_total)
	value.training_focus.clear()
	for focus_row in row.training_focus:value.training_focus[str(focus_row.skill_id)]=int(focus_row.weight)
	value.skill_training.clear()
	for training_row in row.skill_training:value.skill_training[str(training_row.skill_id)]=int(training_row.training_total)
	value.processed_victory_event_ids.clear()
	for event_id in row.processed_victory_event_ids:value.processed_victory_event_ids.append(int(str(event_id)))
	return value

static func wire_error(row:Variant)->String:
	if not row is Dictionary:return "invalid_progression_shape"
	var keys:Array=row.keys();keys.sort()
	if keys!=["processed_victory_event_ids","schema_version","skill_training","training_focus","xp_total"]:
		return "invalid_progression_keys"
	if not _integer(row.schema_version) or int(row.schema_version)!=SCHEMA_VERSION:return "unsupported_progression_schema"
	if not _integer(row.xp_total) or int(row.xp_total)<0 or int(row.xp_total)>RegistryScript.MAX_XP:return "invalid_progression_xp"
	if not row.training_focus is Array or row.training_focus.size()!=RegistryScript.SKILL_IDS.size():return "invalid_progression_focus_shape"
	if not row.skill_training is Array or row.skill_training.size()!=RegistryScript.SKILL_IDS.size():return "invalid_progression_training_shape"
	var focus:={}
	for index in range(RegistryScript.SKILL_IDS.size()):
		var expected_id:String=RegistryScript.SKILL_IDS[index]
		var focus_row:Variant=row.training_focus[index]
		var training_row:Variant=row.skill_training[index]
		if not focus_row is Dictionary or focus_row.keys().size()!=2 \
				or not focus_row.has_all(["skill_id","weight"]) \
				or str(focus_row.skill_id)!=expected_id:return "invalid_progression_focus_row"
		if not training_row is Dictionary or training_row.keys().size()!=2 \
				or not training_row.has_all(["skill_id","training_total"]) \
				or str(training_row.skill_id)!=expected_id:return "invalid_progression_training_row"
		if not _integer(focus_row.weight):return "invalid_progression_focus_value"
		if not _integer(training_row.training_total) or int(training_row.training_total)<0 \
				or int(training_row.training_total)>RegistryScript.MAX_XP:return "invalid_progression_training_value"
		focus[expected_id]=int(focus_row.weight)
	var focus_error:=RegistryScript.focus_error(focus)
	if not focus_error.is_empty():return focus_error
	if not row.processed_victory_event_ids is Array:return "invalid_progression_victory_ids"
	var previous:=0
	for raw_id in row.processed_victory_event_ids:
		if not raw_id is String or not raw_id.is_valid_int():return "noncanonical_progression_victory_id"
		var event_id:=int(raw_id)
		if str(event_id)!=raw_id or event_id<=previous:return "invalid_progression_victory_ids"
		previous=event_id
	return ""

static func _integer(value:Variant)->bool:
	return value is int or (value is float and value==floor(value))
