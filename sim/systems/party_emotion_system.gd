class_name PartyEmotionSystem
extends RefCounted

const ModelScript = preload("res://sim/party_emotion_model.gd")
const EmotionStateScript = preload("res://sim/party_emotion_state.gd")


static func commit_batch(world, event_rows: Array, allow_idle_decay: bool = true) -> bool:
	if world == null or world.party_encounter == null:
		return true
	var source_ids := _source_event_ids(world, event_rows)
	if source_ids.is_empty() and not allow_idle_decay:
		return true
	...
	for row in projection.member_rows:
		...
		if member == null or not world.entities.has(member_id) \
				or not EmotionStateScript.wire_error(row.state_before).is_empty() \
				or not EmotionStateScript.wire_error(row.state_after).is_empty():
			return false
		var magnitude := _total_change(row.state_before, row.state_after)
		var event = world.emit_event("party.emotion_changed", member_id, -1,
			world.entities[member_id].position, magnitude, cause_id, {
		...
		if event == null:
			return false
		member.emotion_state = EmotionStateScript.from_dict(row.state_after)
		changed = true
	if changed:
...
static func _total_change(before: Dictionary, after: Dictionary) -> int:
	var total := 0
	for index in range(EmotionStateScript.EMOTION_IDS.size()):
		total += absi(int(after.channels[index].intensity) \
