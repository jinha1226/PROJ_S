...
func preview_choice(choice_action: String) -> Dictionary:
	...
	if choice_action == "GIVE_POTION":
		var potion_id := _healing_potion_instance_id(state.protagonist_inventory)
		if potion_id.is_empty(): return _rejected("opening_healing_potion_missing")
		var item_preview: Dictionary = ItemOperationsScript.preview_use(
			state.protagonist_inventory, potion_id)
...
func commit_preflighted_choice(preview: Dictionary) -> Dictionary:
	...
	if choice_action == "GIVE_POTION":
		...
		var consumed: Dictionary = ItemOperationsScript.commit_use(
			state.protagonist_inventory, instance_id)
		...
		state.protagonist_inventory = consumed.inventory
