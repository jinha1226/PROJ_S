...
const ROLE_TEXT := {
	"PACK":{2:"파티 전원 공격력 +10%",4:"파티 전원 공격력 +20%, 동료 최대 HP +20%",6:"파티 전원 공격력 +30%, 처치할 때마다 파티 HP 5% 회복"},
	"BERSERK":{2:"공격력 +15%",4:"공격력 +30%, 체력 절반 이하에서 행동 속도 +20%",6:"공격력 +50%, 처치하면 HP 10% 회복"},
	"AMBUSH":{2:"치명타 +10%",4:"치명타 +20%, 치명타 피해 +50%",6:"치명타 +30%, 치명타 피해 +50%, 체력이 가득한 대상에게는 항상 치명타"},
	"GUARD":{2:"방어 +3",4:"방어 +6, 막기 +10",6:"방어 +10, 막기 +20, 인접 아군 받는 피해 −15%"},
	"ARCHER":{2:"원거리 사거리 +1",4:"원거리 피해 +25%",6:"원거리 피해 +40%, 25% 확률로 한 번 더"},
	"CASTER":{2:"주문력 +15%",4:"주문력 +30%, 라운드마다 MP +2",6:"주문력 +50%, 주문 실패 없음"}}
const TEXT := {
...
const GUARD_ARMOUR := {2:3,4:6,6:10}
const GUARD_BLOCK := {2:0,4:10,6:20}
