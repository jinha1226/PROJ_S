	...
	for row in intents:
		...
		if intent == "ATTACK":
			...
			for corner in [Vector2(-1,-1),Vector2(1,-1),Vector2(-1,1),Vector2(1,1)]:
				...
				draw_line(marker,marker-Vector2(corner.x*4,0),color,1.0,true)
				draw_line(marker,marker-Vector2(0,corner.y*4),color,1.0,true)
			...
			draw_colored_polygon(PackedVector2Array([tip,tip-direction*7+normal*4,tip-direction*7-normal*4]),color)
		elif intent == "PROTECT":
			var center: Vector2 = board.cell_center(cell)
			draw_line(center+Vector2(-5,0),center,color,2,true)
			draw_line(center,center+Vector2(5,0),color,2,true)
			draw_line(center+Vector2(-5,0),center+Vector2(-4,5),color,2,true)
			draw_line(center+Vector2(5,0),center+Vector2(4,5),color,2,true)
			draw_line(center+Vector2(-4,5),center+Vector2(0,8),color,2,true)
			draw_line(center+Vector2(4,5),center+Vector2(0,8),color,2,true)
