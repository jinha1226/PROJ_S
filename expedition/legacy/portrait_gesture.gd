...
func _blocked(host)->bool:
	return host.session == null or host.details_popup.visible or host.map_popup.visible or host.log_popup.visible
...
func handle(host,event:InputEvent)->bool:
	...
	if actor_id<0:
		...
		for card in host.root_layout.find_children("MemberCard*","Button",true,false):
		...
		if released:
			...
			if activate:
				if held:host.show_character(id,"상태")
				else:host.select_actor(id)
	host.get_viewport().set_input_as_handled();return true
...
func tick(host)->void:
	...
	if Time.get_ticks_msec()-started>=HOLD_MSEC:
		consumed=true
		host.show_character(actor_id,"상태")
