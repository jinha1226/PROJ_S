...
func _build_ui()->void:
	...
	root.offset_left=6; root.offset_right=-6; root.offset_top=4; root.offset_bottom=-4; root.add_theme_constant_override("separation",4); add_child(root)
	phase_label=Label.new(); phase_label.name="PhaseStatus"; phase_label.custom_minimum_size.y=48; phase_label.add_theme_font_size_override("font_size",FONT_KEY)
	...
	grid.world_cell_pressed.connect(_on_cell); grid.actor_pressed.connect(_on_actor); root.add_child(grid)
	cards=HBoxContainer.new(); cards.name="PartyCards"; cards.custom_minimum_size.y=160
	cards.add_theme_constant_override("separation",4); root.add_child(cards)
	...
	info_scroll.horizontal_scroll_mode=ScrollContainer.SCROLL_MODE_DISABLED; root.add_child(info_scroll)
...
func _refresh()->void:
	...
	grid.set_observation(session.observe_party_world(),ghosts); grid.set_selection(selected_member_id,selected_target_id)
	grid.set_intent_overlays(session.turn_intent_overlays() if str(status.view_mode)=="COMBAT" else [])
...
func _combat_deck(status:Dictionary,preview:Dictionary)->void:
	...
	var clear:=_add_button(controls,"자동 제안 복원","OverrideClear",_on_override_clear); clear.disabled=selected_member_id==int(status.protagonist_id)
	var confirm:=_add_button(controls,"파티 턴 일괄 확정","TurnConfirm",_on_turn_confirm); confirm.disabled=not bool(preview.get("accepted",false))
	_selected_detail()
...
func _select_member(member_id:int,display_name:String)->void:selected_member_id=member_id; selected_target_id=-1; notice_text="%s 선택"%display_name; _clear_move_preview(); _request_refresh()
func _on_explore(direction:Vector2i)->void:_record_result(session.commit_exploration_direction(direction)); _request_refresh()
func _on_preset(preset:String)->void:
	var result:Dictionary=session.preview_deployment(preset,session.available_companion_ids()); notice_text="%s 대형: %s"%[{"WEDGE":"쐐기","LINE":"횡대","COLUMN":"종대"}[preset],str(result.message)]; _request_refresh()
func _on_deploy_confirm()->void:
	...
	_request_refresh()
func _on_actor_hold()->void:_clear_move_preview(); _record_result(session.set_actor_action(selected_member_id,"HOLD")); _request_refresh()
func _on_override_clear()->void:_clear_move_preview(); _record_result(session.clear_companion_override(selected_member_id)); _request_refresh()
func _on_turn_confirm()->void:
	...
	if not bool(current.get("accepted",false)):notice_text=str(current.get("message","행동을 확인하세요."))
		...
		_record_result(session.commit_turn()); _clear_move_preview()
...
func _on_cell(position:Vector2i)->void:
	...
	if status.view_mode=="EXPLORATION":
		...
		if same and pending_move_valid:
			var result:Dictionary=session.commit_exploration(CommandScript.move_to(hero,position)); _clear_move_preview(); _record_result(result)
		else:
			...
			elif str(preview.get("reason",""))=="move_not_adjacent":notice_text="장거리 이동은 아직 지원하지 않습니다. 인접한 8방향 칸을 선택하세요."
	...
	if same and pending_move_valid:_record_result(session.set_actor_action(selected_member_id,"MOVE",[position.x,position.y])); _clear_move_preview()
	else:
		...
		pending_move_valid=bool(preview.get("accepted",false)); pending_move_cost=int(preview.get("total_time_cost",0)); notice_text="" if pending_move_valid else str(preview.get("message","이 칸으로 이동할 수 없습니다."))
	_request_refresh()
func _on_actor(entity_id:int)->void:
	...
	if status.view_mode=="EXPLORATION" and entity_id==int(status.protagonist_id):
		...
		if pending_move_mode=="EXPLORATION" and pending_exploration_wait:
			var result:Dictionary=session.commit_exploration(CommandScript.wait(entity_id)); _clear_move_preview(); _record_result(result)
		else:
	...
	if entity_id in status.visible_enemy_ids:
		...
		if status.view_mode=="COMBAT" and not bool(status.terminal):_record_result(session.set_actor_action(selected_member_id,"MELEE",[],entity_id))
		_request_refresh(); return
	if entity_id in status.party_member_ids:selected_member_id=entity_id; selected_target_id=-1; notice_text="파티원을 선택했습니다."; _clear_move_preview(); _request_refresh()
...
func _record_result(result:Dictionary)->void:notice_text="" if bool(result.get("accepted",false)) else str(result.get("message",session.reason_message(str(result.get("reason","invalid_party_action")))))
...
func _apply_portrait_budget()->void:
	var wide:=size.x>=450.0; phase_label.custom_minimum_size.y=52 if wide else 48
	grid.custom_minimum_size=Vector2(405,405) if wide else Vector2(348,348)
