...
func _draw():
	...
	if texture!=null:
		var side:=minf(size.x-6,size.y-32)
		draw_texture_rect(texture,Rect2(Vector2((size.x-side)/2,2),Vector2(side,side)),false)
	...
	for i in range(2):
		...
		var rect:=Rect2(4,size.y-27+i*13,size.x-8,11)
