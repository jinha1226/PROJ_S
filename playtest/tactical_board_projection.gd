extends RefCounted
## Parallel 2:1 diamond projection for the existing scrolling world grid.
## This module transforms presentation coordinates only; it never limits the map
## to a room or changes simulation movement.

static func half_width(viewport:Rect2,count:int)->float:
	return minf(viewport.size.x/(maxi(1,count)*1.35),
		viewport.size.y/maxi(1,count))
...
static func project(point:Vector2,viewport:Rect2,count:int)->Vector2:
	...
	return viewport.get_center()+Vector2((local.x-local.y)*h,
		(local.x+local.y)*h*0.5)

static func unproject(pixel:Vector2,viewport:Rect2,count:int)->Vector2:
	var p:Vector2=(pixel-viewport.get_center()) \
		/maxf(0.001,half_width(viewport,count))
	return Vector2(p.y+p.x*0.5,p.y-p.x*0.5) \
		+Vector2.ONE*(float(count)*0.5)
