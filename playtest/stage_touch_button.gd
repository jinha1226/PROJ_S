...
var _skin_accent:=false
var _finger:=-1
var _origin:=Vector2.ZERO
var _cancelled:=false
var _held:=false
var _serial:=0
func set_skin_accent(value:bool)->void:
	_skin_accent=value
	_update_skin()
func _update_skin()->void:
	ButtonSkin.apply(self,_skin_accent,_finger>=0 and not _cancelled and not _held)
	...
	if event is InputEventScreenTouch:
		...
		if event.pressed and _finger<0:
			...
			get_tree().create_timer(0.55).timeout.connect(func():
				if serial==_serial and _finger>=0 and not _cancelled:
					_held=true;_update_skin();held.emit())
