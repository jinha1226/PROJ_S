...
static func surface()->StyleBoxFlat:
	...
	style.border_color=Color("414954");style.set_border_width_all(1);style.set_corner_radius_all(16)
...
static func style_button(button:Button)->void:
	for state in ["normal","hover","pressed","focus","disabled"]:
		...
		style.bg_color=Color("514735") if state=="pressed" else Color("29323b")
		if state=="hover":style.bg_color=Color("343e48")
		if state=="disabled":style.bg_color=Color("232930")
	...
	button.add_theme_color_override("font_disabled_color",Color("78818a"))
...
static func mount(panel,sections:Array)->void:
	...
	var header:=VBoxContainer.new();header.add_theme_constant_override("separation",10);hud.add_child(header)
	...
	for resource in ["TIMBER","STONE","HERBS","FOOD"]:
		var row:=HBoxContainer.new();row.add_theme_constant_override("separation",8);row.size_flags_horizontal=Control.SIZE_EXPAND_FILL;resources.add_child(row)
		row.add_child(icon(resource,24))
		var count:=Label.new();count.name="SettlementCount"+resource;count.text="0";count.add_theme_font_size_override("font_size",20);row.add_child(count)
	...
	feedback.max_lines_visible=2;feedback.add_theme_font_size_override("font_size",16);feedback.add_theme_color_override("font_color",Color("a8b2b7"));header.add_child(feedback)
	...
	sheet.offset_top=sheet.offset_bottom-minf(330,stage.custom_minimum_size.y*0.48)
	var contents:=VBoxContainer.new();contents.name="SettlementSheetContent";contents.add_theme_constant_override("separation",12);sheet.add_child(contents)
	...
	for i in range(sections.size()):
		...
		var stack:=VBoxContainer.new();stack.size_flags_horizontal=Control.SIZE_EXPAND_FILL;stack.add_theme_constant_override("separation",12);scroll.add_child(stack)
	...
	var bar:=HBoxContainer.new();bar.name="BaseSectionTabs";bar.add_theme_constant_override("separation",8);nav.add_child(bar)
	...
	for entry in entries:
		...
		var text:=Label.new();text.text=str(entry[0]);text.horizontal_alignment=HORIZONTAL_ALIGNMENT_CENTER;text.mouse_filter=Control.MOUSE_FILTER_IGNORE;text.add_theme_font_size_override("font_size",18);column.add_child(text)
