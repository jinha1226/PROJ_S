...
func sync(host):
	...
	if signature!=_signature:
		...
		for row in members:
			...
			portrait.pressed.connect(func():
				if r.phase=="DEPLOYMENT":host._select_member(id,str(row.display_name));host._request_refresh()
				else:host._open_member_detail(id,"STATUS"))
	...
	if active:
		...
		if r.get("individual",false) and r.phase!="DEPLOYMENT":
			...
			proceed.text="적 턴 진행" if not current.get("ally",false) else "턴 종료"
