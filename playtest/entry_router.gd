...
func _ready() -> void:
	...
	var expedition_shell := "--expedition-shell" in OS.get_cmdline_user_args()
	if OS.has_feature("web"):
		...
		expedition_shell = bool(JavaScriptBridge.eval(
			"new URLSearchParams(window.location.search).get('demo') === 'expedition'"))
	var scene_path := "res://game/expedition_shell/main.tscn" if expedition_shell \
