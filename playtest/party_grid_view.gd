...
signal actor_inspect_requested(entity_id: int)
...
const RegularFont:FontFile=preload("res://assets/fonts/Galmuri14.ttf")
...
const TORCH_FLICKER_QUANTUM_MS := 260 # Deliberate firelight pulse; frozen when wide.
...
const CAMPFIRE_LIGHT_RADIUS_CELLS := 4.2
const CAMPFIRE_LIGHT_INTENSITY := 100
...
func set_observation(observation: Dictionary, ghosts: Array = []) -> void:
	...
	_ghosts.sort_custom(func(a,b):
		if int(a.get("roster_slot",99)) != int(b.get("roster_slot",99)): return int(a.get("roster_slot",99)) < int(b.get("roster_slot",99))
		return int(a.get("entity_id",-1)) < int(b.get("entity_id",-1)))
	_reconcile_actor_last_facing(previous_actors)
...
func actor_last_facing_state()->Dictionary:
	return _actor_last_facing.duplicate(true)
...
func _reconcile_actor_last_facing(previous_actors:Dictionary)->void:
	...
	for actor in _actors:
		...
		var previous:Dictionary=previous_actors[entity_id]
		var logical_delta:=_logical_position_from_actor(actor)-_logical_position_from_actor(previous)
		var display_delta:=_position_from_actor(actor)-_position_from_actor(previous)
		var delta:=logical_delta if logical_delta!=Vector2i.ZERO else display_delta
		if delta!=Vector2i.ZERO and maxi(absi(delta.x),absi(delta.y))==1:
			_actor_last_facing[entity_id]=Vector2i(signi(delta.x),signi(delta.y))
	for raw_id in _actor_last_facing.keys():
		if not next_ids.has(int(raw_id)):_actor_last_facing.erase(raw_id)
...
func actor_draw_spec(actor:Dictionary,ghost:bool=false,sample_time_ms:int=-1)->Dictionary:
	...
	if not ghost and _actor_motions.has(entity_id):
		...
		var last_facing:Vector2i=_actor_last_facing[entity_id]
		projected["facing"]=[last_facing.x,last_facing.y]
	if bool(actor.get("guarded",false)) and not projected.has("visual_stance"):
...
func nearby_actor_at_pointer(pointer:Vector2)->int:
	...
	if not spec.visible or not Rect2(spec.bounds).has_point(pointer):return -1
	for row in spec.rows:
		if Rect2(row.hit_rect).has_point(pointer):return int(row.entity_id)
	return 0 # Panel padding consumes input, too.

func capture_nearby_list_input(event:InputEvent)->bool:
	# Claim the HUD before the sandbox's map movement, pinch, or battle drag
	# handlers. Route all devices through the same release/slop semantics.
	if modal_open or not is_visible_in_tree():return false
	if not event is InputEventScreenTouch and not event is InputEventScreenDrag \
			and not event is InputEventMouseButton and not event is InputEventMouseMotion:return false
	var captured:=_pointer_gesture_active and _pointer_gesture_target_kind=="INSPECT"
	var starts:bool=(event is InputEventScreenTouch and event.pressed) or (
		event is InputEventMouseButton and event.button_index==MOUSE_BUTTON_LEFT and event.pressed)
	var local_event:InputEvent=event.xformed_by(get_global_transform_with_canvas().affine_inverse())
	if not captured and (not starts or nearby_actor_at_pointer(local_event.position)<0):return false
	if captured and event is InputEventScreenTouch and event.pressed and event.index!=_pointer_gesture_index:
		cancel_pointer_gesture()
	else:_gui_input(local_event)
...
func _rebuild_torch_cache()->void:
	...
	for cached_value in _static_projection_cache.values():
		...
		if state=="VISIBLE" and (animate_passive_terrain \
				...
				or fire>0 or campfire):_visible_environment_count+=1
		if fire<=0 and not campfire:continue
		var position:Vector2i=cached.get("position",Vector2i(-1,-1))
		if position!=Vector2i(-1,-1):
			_fire_light_positions.append({"position":position,
				"intensity":CAMPFIRE_LIGHT_INTENSITY if campfire else fire,
				"radius_cells":CAMPFIRE_LIGHT_RADIUS_CELLS if campfire \
				else FIRE_LIGHT_RADIUS_CELLS,"campfire":campfire,
...
func _begin_pointer_gesture(kind:String,pointer_index:int,pointer:Vector2)->void:
	...
	var nearby_id:=nearby_actor_at_pointer(pointer)
	if nearby_id>=0:
		...
		_pointer_gesture_start=pointer;_pointer_gesture_last=pointer
		_pointer_gesture_target_kind="INSPECT";_pointer_gesture_target_actor_id=nearby_id
		_pointer_gesture_long_fired=false;_pointer_gesture_cancelled=false
		pointer_gesture_started.emit();return
...
func _finish_pointer_gesture(kind:String,pointer_index:int,pointer:Vector2,cancelled:bool)->void:
	...
	if target_kind=="INSPECT":
		var clicked:=not cancelled and not gesture_cancelled and pointer.distance_to(_pointer_gesture_start)<=POINTER_SLOP_PX \
			and nearby_actor_at_pointer(pointer)==target_actor_id
		_reset_pointer_gesture()
		if clicked and target_actor_id>0:actor_inspect_requested.emit(target_actor_id)
		pointer_gesture_finished.emit("SHORT_TAP" if clicked else "CANCELLED")
		return
...
func _draw_topdown_fixed_front_actor(actor:Dictionary,ghost:bool,
	...
	if body_texture!=null and bool(spec.get("walk_active",false)) \
			and Vector2(spec.get("source_canvas_size",Vector2.ZERO))==Vector2(24,24):
		_draw_fixed_front_walking_body(body_texture,bounds,modulate,
			int(spec.get("walk_phase",0)))
	...
	if foreground_texture!=null:
		draw_texture_rect(foreground_texture,bounds,false,modulate)
...
func _draw_fixed_front_walking_body(texture:Texture2D,bounds:Rect2,
		modulate:Color,walk_phase:int)->void:
	# Only the bottom four source pixels alternate. Head and torso keep the idle
	# transform, while armor and weapon remain on the same interpolated root.
	var upper_source:=Rect2(0,0,24,20)
	var upper_bounds:=Rect2(bounds.position,Vector2(bounds.size.x,bounds.size.y*20.0/24.0))
	draw_texture_rect_region(texture,upper_bounds,upper_source,modulate)
	var source_pixel:=bounds.size.x/24.0
	for half in range(2):
		var source:=Rect2(float(half*12),20.0,12.0,4.0)
		var destination:=Rect2(bounds.position+Vector2(bounds.size.x*0.5*half,
			bounds.size.y*20.0/24.0),Vector2(bounds.size.x*0.5,bounds.size.y*4.0/24.0))
		if half==walk_phase:destination.position.y-=source_pixel
...
func fixed_front_actor_render_spec(actor:Dictionary,ghost:bool=false,
	...
	var motion:Dictionary=actor_motion_draw_spec(entity_id,sample_time_ms) if entity_id>0 \
		else {"active":false,"stride_sign":0}
	var walk_active:=not ghost and life_state=="ACTIVE" \
		and bool(layer_spec.get("supports_walk",false)) and bool(motion.get("active",false))
	var walk_phase:=0 if int(motion.get("stride_sign",-1))<0 else 1
	...
	return layer_spec.merged({"visible":true,"uses_sprite":true,"bounds":bounds,
		...
		"walk_active":walk_active,"walk_phase":walk_phase,
		"walk_source_foot_rows":Rect2i(0,20,24,4),"walk_body_bob_px":0.0,
...
func topdown_ascii_actor_render_spec(actor:Dictionary,ghost:bool=false,
		sample_time_ms:int=-1,camera_offset:Vector2=Vector2.ZERO)->Dictionary:
	var position:=_position_from_actor(actor)
	if not is_world_cell_visible(position):return {"visible":false}.duplicate(true)
	var entity_id:=int(actor.get("entity_id",-1))
	...
	if not ghost and entity_id==_hero_camera_actor_id:center-=camera_offset
	var cell:=cell_size_px()
	# Art may overlap neighboring ground visually, while logical selection and
	# occupancy remain the existing one-cell actor authority.
	var extent:=Vector2(cell*1.45,cell*1.55)
	var foot_y:=center.y+cell*0.42
	var bounds:=Rect2(Vector2(center.x-extent.x*0.5,foot_y-extent.y),extent)
	var style:=actor_draw_spec(actor,ghost,sample_time_ms)
	var spec:=_asciident_actor_render_spec(actor,bounds,style)
	spec["style"]=style
	spec["foot_y"]=foot_y
...
static func directional_darkness_distance(offset_cells:Vector2,facing:Vector2i,
	...
	return _directional_darkness_distance(offset_cells,Vector2(facing),torch_lit)
...
func _fire_alpha_at(sample_cell:Vector2i,point:Vector2)->float:
	...
	for source in _visible_light_sources_by_cell.get(_key(sample_cell),[]):
		...
		var distance:=world_to_pixel_center(source_position).distance_to(point) \
			/maxf(1.0,cell_size_px())
		if distance>radius:continue
		var source_strength:=clampf(float(source.get("intensity",100))/100.0,0.0,1.0)
		var alpha:=lerpf(0.94,campfire_darkness_sample(distance,radius),source_strength)
		best=alpha if best<0.0 else minf(best,alpha)
	return best
