...
static func use(session,instance_id:String,selection:Dictionary={})->Dictionary:
	...
	if definition_id.begins_with("POTION_") and not selection.is_empty():return preload("res://playtest/consumable_utility_service.gd").use(session,instance_id,selection)
