...
파티 전투의 기본 조작은 동료별 명령이나 전투마다 다시 고르는 전술 프리셋이 아니다.
플레이어는 주인공만 조작하고 동료는 아래 흐름으로 완전 자동 행동한다.

```text
개별 시야 → 파티 정보 공유 → 자동 경고 → 플레이어 행동을 암묵적 지시로 해석
```

- 각 동료는 자신의 시야와 감각으로 적을 먼저 발견할 수 있다. 발견 성공 여부에는 HEXACO
  성향을 사용하지 않는다.
...
- HEXACO는 탐지나 경고를 확률적으로 누락시키지 않는다. 성향은 동일한 정보 위에서 보호,
  교전, 공포 이탈 같은 반응의 임계값에만 영향을 준다.

제품 조작에 남기는 예외 명령은 파티 전체에 적용되는 `공격 대상 지정`, `후퇴`, `공격 중지`,
`자리 지키기`, `따라오기` 다섯 개뿐이다. 평상시에는 사용할 필요가 없어야 하며, 개별 동료
override는 테스트·디버그 관찰용 API로만 유지하고 제품의 기본 전투 동선에서는 노출하지 않는다.
상단 `NEXT` 요약과 유닛별 가는 표적선은 명령 입력기가 아니라 자동 판단을 설명하는 표면이다.

예외 명령은 `party.command_issued` 권위 사건으로 기록한다. 공격 대상 지정은 자율 전술의 공통
