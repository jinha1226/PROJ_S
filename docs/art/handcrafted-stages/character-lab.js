const CharacterLab=(()=>{
 ...
 function draw(){
  ...
  for(const variant of ['base','equipped']){
   ...
   if(document.querySelector('#skeleton')?.checked&&CharacterArt.drawSkeleton)CharacterArt.drawSkeleton(ctx,variant,direction,pose,160,213,3);
  ...
  for(let p=0;p<poses.length;p++){const ctx=poses[p].getContext('2d');ctx.clearRect(0,0,128,128);CharacterArt.draw(ctx,'base',direction,p,64,112,2);}
...
for(const name of CharacterArt.poseNames||['정지','보행 A','보행 B','타격']){const figure=document.createElement('figure'),canvas=document.createElement('canvas'),caption=document.createElement('figcaption');canvas.width=128;canvas.height=128;caption.textContent=name;figure.append(canvas,caption);document.querySelector('#poses').append(figure);poses.push(canvas);}
 ...
 if(document.querySelector('#skeleton'))document.querySelector('#skeleton').onchange=draw;
