...
The first two rows are action and supply icons. The third row contains frame concepts; these large cells are not used as button backgrounds because their edges overlap the grid and do not scale cleanly to 44px buttons. The fourth row contains compact utility symbols for map, camp, retreat, log, add, and close actions.
...
The generated sheet is 1536×1024 with six 256px columns, but the artwork does not respect the nominal row boundaries. `expedition/art/mobile_art.gd` crops icons at the actual transparent row gaps and builds a clean 24px frame texture for each button state. `expedition/ui/main.gd` uses those frames with 6px nine-slice margins. Floor action and supply buttons use the cropped atlas icons in `expedition/ui/screens/floor_hud.gd`.
