...
The first two rows are action and supply icons. The third row supplied the original button frames in `assets/ui/button-frames-8bit-v1.png`. The active nine-slice frames are the sharper six-state `assets/ui/button-frames-8bit-v2.png`, built by `tools/art/build_button_frames_8bit.py` with fixed 10px corners. The fourth atlas row contains compact utility symbols for map, camp, retreat, log, add, and close actions.
...
The generated sheet is 1536×1024 with six 256px columns, but the artwork does not respect the nominal row boundaries. `expedition/art/mobile_art.gd` crops icons at the actual transparent row gaps and reads the separate v2 button frames. `expedition/ui/main.gd` uses those frames with 10px nine-slice margins. Floor action and supply buttons use the cropped atlas icons in `expedition/ui/screens/floor_hud.gd`.
