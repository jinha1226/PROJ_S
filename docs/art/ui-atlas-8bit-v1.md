...
The first two rows are action and supply icons. The third row contains complete frame samples used as `StyleBoxTexture` sources. The fourth row contains compact utility symbols for map, camp, retreat, log, add, and close actions.
...
The generated sheet is normalized to a 6×4 256-pixel grid for the Godot atlas regions. `expedition/art/mobile_art.gd` exposes the regions, and `expedition/ui/main.gd` applies the frame cells to every Button state. Floor action and supply buttons use the new icons in `expedition/ui/screens/floor_hud.gd`.
