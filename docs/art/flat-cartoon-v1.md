...
- 그라데이션 없는 평면 채색. 소품과 아이콘은 참고 화면의 통처럼 오른쪽에 어두운 세로 띠 하나만 둔다.
...
- 바닥에는 반투명 타원 그림자. 넓은 면은 격자선 없이 한 색으로 칠하고, 같은 색의 어두운 얼룩과 검은 잉크 낙서(ᴍ 모양 풀, 조약돌)로만 질감을 준다.
- 벽은 평면 석판색에 큰 블록을 얇은 어두운 선으로만 나눈다. 굵은 검은 선은 벽과 바닥의 큰 경계에만 쓴다.
- 격자선이 없으므로, 게임에 넣을 때는 이동 가능 칸이나 공격 범위만 필요할 때 표시한다.
