...
2026-09-23: 기존 먹선 원본 parts.png로 복원했다. 인간 전체 표시 배율은 2.4에서 20% 줄인 1.92로 조정했고, 머리는 최초 92×139에서 128.8×194.6으로 40% 확대했다. 머리 중심 x=128, 하단 y=163을 유지한다. 두 배율은 곱해 적용된다. 인게임 배치·docs/art/catalog.json·HTML 미리보기도 동기화했다.

선택한 짧고 넓은 토르소와 각진 먹선 스타일을 인간 주인공 아린에 적용한다.
원본 시안은 [torso-rimworld-darkest-v1.png](torso-rimworld-darkest-v1.png).

- 현재 파츠 원본: `assets/characters/ink-torso-v1/parts.png` (1254×1254 RGBA). 단순화 시안 parts-readable-v2.png는 비교용으로 보존한다.
...
- 보드와 이동 잔상은 하단을 고정하고 위로 확대한다. 레이어 인간은 기존 표시 영역의 1.92배, 여백이 적은 기존 동료는 1.67배로 표시한다. 타일 점유와 판정은 유지한다.
