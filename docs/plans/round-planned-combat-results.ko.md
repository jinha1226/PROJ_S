...
후속 검토/수정: `round-planned-combat-review.ko.md`에 초기 배포 차단 문제와 수정 내용을 기록했다. 순서창을 포함한 UI 통합, 파싱 오류, 소진 이동 예산 복구, 아이소매트릭 선택 테두리를 후속 커밋에 포함한다. 아래 '남은 인수 검증' 중 실제 모바일/장시간 성능 등은 여전히 별도 확인 대상이다.
