...
- 표적 = 공통 표적. **자기 방어가 공통 표적에 앞선다**: 이 멤버의 근접 사거리 안에 적이 있으면 그 적을 친다(공통 표적이면 물론, 아니어도 — 이미 자기를 때리고 있는 적을 두고 다른 칸으로 걸어가지 않는다). 인접한 적이 여럿이면 공통 표적이 먼저다(`adjacent_foe`). **ATTACK**(점수 100 + 피해). 인접한 적이 하나도 없을 때만 표적에 인접한 빈 칸으로 **최단 경로 MOVE**(점수 80). 경로가 없으면 가장 가까운 적으로 같은 처리.
- 예고 칸 회피는 **하지 않는다**(`posture`가 −60 이하이고 예고 칸에 서 있을 때만 예외: 그 라운드는 공격하지 않고 예고 칸에서 한 칸 비켜서는 MOVE(점수 60)로 대신한다 — ATTACK 후보에 나란히 더해지는 것이 아니라 이를 대체하는 조기 반환이다). 불 칸은 항상 피한다.
...
- **근접 접촉 중에는 사거리 3 이상 파츠 규칙을 건너뛴다**: 적이 이 멤버의 근접 사거리 안에 있으면 규칙 단계(3단계)의 후보 풀에서 `range ≥ 3`인 파츠를 걸러낸다(`Tactics.choose`). 붙은 채로 쏘는 대신 먼저 거리를 벌린다는 뜻이고, `d < 2`의 이탈 후보(120)가 실제로 선택되게 하는 장치다.
...
- 우선순위: (a) `P`가 치명 위기(`lethal_threat ≥ hp`)면 엄호 규칙(3단계, ALLY_LETHAL) — 엄호 파츠가 없으면 `P`와 위협 사이 칸으로 MOVE(130). (b) 위협이 인접이면 ATTACK(100). (c) 위협이 있으면 위협과 `P` 사이 칸(`P`에서 위협 방향 1칸)으로 MOVE(90). (d) 위협이 없으면 `P` 인접 칸 유지: `P` 곁에 있으면 **자기 근접 사거리 안의 적**이 있을 때 ATTACK(60) — 공통 표적이 먼저지만 공통 표적이 아니어도 자기를 때리는 적은 친다(`adjacent_foe`, §2.1과 같은 자기 방어 우선) — 사거리 안에 적이 없으면 **동반 전진**(아래), 그래도 후보가 없으면 WAIT(40); `P` 곁이 아니면 `P` 인접 칸으로 MOVE(80).
- **동반 전진(d-1)**: `P`가 위협받지 않고 자기 사거리 안에도 적이 없을 때, 공통 표적 쪽으로 MOVE(점수 50 — 사거리 안 적의 ATTACK 60보다 낮고 대기 40보다 높다). 목표 칸은 **공통 표적 인접 빈 칸 ∩ `P`로부터 체비쇼프 거리 `keep+1` 이내**이고, 그 교집합이 비면 표적으로 가는 경로의 첫 걸음 중 `keep+1` 안에 남는 것을 쓴다(`steps_toward`). 전진은 언제나 호위 범위 안에서만 일어나므로 "전진"이 "이탈"이 되지 않는다. **서로를 호위하는 둘 이상이 함께 전진하는 근거가 이 후보다** — 이것이 없으면 상호 호위 파티는 아무도 움직이지 않고 전투가 성립하지 않는다(G7 `opt_archers` 0.07).
...
- 단 하나의 예외: **거리형이 근접 접촉 중이면 사거리 3 이상 파츠는 규칙 단계의 후보 풀에서 빠진다**(§2.2). 태세가 규칙을 이기는 유일한 자리이며, 그 외에는 규칙이 언제나 태세보다 앞선다.
- 자기 방어는 공통 표적보다 앞선다(§2.1·§2.3): 근접 사거리 안에 적이 있으면 돌격형·호위형은 그 적을 친다. 공통 표적은 동점일 때의 우선 대상이지 이동 강제가 아니다.
