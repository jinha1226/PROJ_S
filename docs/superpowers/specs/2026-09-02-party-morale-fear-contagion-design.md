...
- 직접 충격은 자기 물리/다운 피해, 아군 다운·사망, 적 사망만 읽는다. 같은 이벤트
  ...
  정렬한다.
- 양의 직접 충격만 거리 4 이내 생존 배치 아군에게 전염된다. 수신자의 HEXACO E/C에서
  도출한 회복탄력성이 높을수록 작고, 한 배치 전염량은 180 이하이다.
...
- 적 진영 사기와 도주, 새 행동 타입, RNG 기반 공포 판정은 여전히 비범위다.
- 화면 밖 전투는 별도 P4에서 P4-1 forecast와 P4-2a 독립 권위 리듀서까지 완료됐다.
  제품 `WorldState` bridge와 상세 복귀는 P4-2b/P4-3 범위다.
