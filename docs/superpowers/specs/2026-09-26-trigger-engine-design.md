...
## 부위·장비 후속 계약 (2026-09-26)

- `EffectEngine.effects`는 장착 부위와 장착 장비의 효과 ID를 합치며 같은 ID는 한 번만 실행한다. 장비 수치 속성은 별도로 합산한다.
- 추가 이벤트: `MOVED`, `DOT_TICK`, `HEALED`. `HEALED`는 실제 회복량·초과 회복·흡혈 여부와 회복자를 전달한다.
- `effect_mods`는 출처별 수치·쌓임 수·만료 tick을 가진다. 대상의 방어 감소와 회복 후 방어 증가는 이 상태를 읽는다. 방어 최종값은 0 이상이다.
- 추가 조건은 이전 라운드의 피격/이동, 이동 후 공격, 공격자의 중독, 표적의 물기·해로운 상태, 외부 회복·흡혈·소환수 공동 표적이다. 조준은 완료한 라운드의 이동 여부를 본다.
- `next_action` 쌓임은 소유자의 다음 성공한 행동에 소모한다. 다른 인물의 행동 시작에는 유지하며, 주문도 같은 소비 경로를 쓴다. 그 행동에서 새로 얻은 쌓임은 보존한다.
- 추가 수치 키: 형태별 부상 확률·부상 저항·타격 방어 적용률·피부 보정·선호 부위 확률, 상태별 지속, 조준/수호/독 쌓임 최대, 상태당 피해, 치명타/명중, 회복량, 소환 시간·수, 반경, 처치 발동 확률, 이동 지연, 이동 후 공격 제한, 피해 분담·최대 자원 대가.
- 지연 폭발은 지점·진영·처치자·피해를 보존하고 한 번만 처리하며 층 전환 시 제거한다. 피해는 2차 피해이며 자체 부상이나 반응을 만들지 않는다.
- 주문 소환과 처치 소환은 소환자별 같은 한도를 공유한다. 별도 지정된 수명에도 장비의 수명 보정이 적용된다.
- 상태가 만료되면 독/골절 보조값도 지우고 추가 틱을 처리하지 않는다. 표식은 해로운 상태 수와 상태 부여 이벤트에 포함한다.
