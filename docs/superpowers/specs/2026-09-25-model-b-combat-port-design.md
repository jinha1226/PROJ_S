...
- 새 Run → 목록이 아니라 **장면**이다: 던전 포탈 앞 광장(고정 템플릿 `portal_plaza`, 24×16, 가운데 위에 포탈 `O`, 모닥불 2, 천막 3, 바닥 `stone/rubble`)에 고정 주인공(아린)과 이번 Run의 NPC 명부 **30명**이 서 있다. 기존 보드가 그대로 그린다(토큰·이름표·활동 문구, 시야 제한 없음 — 광장은 전부 보임).
- NPC는 광장에서 **어슬렁거린다**: 활동 모드 `MINGLE`(광장 전용) — 모닥불·천막·자기 2인 조 파트너 주변 2칸 안에서 시드 기반으로 한 걸음씩(주인공이 한 걸음 걸을 때마다 tick이 흐르고 스케줄러가 NPC를 움직인다). 전투·영입·소음·잠듦 없음. 파트너 인접 유지 규칙은 그대로라 2인 조가 같이 다니는 게 보인다.
- 주인공은 광장을 걸어 다닐 수 있다(탭 이동). NPC 토큰 탭(인접) → 기존 NPC 팝업(이름·종족·성격 한 줄·들고 있는 무기; 영입 버튼 없음). 주인공 카드 탭 → **종족**(인간/드워프/엘프 — HP·MP·STR/INT/DEX·적성)과 **시작 장비 10개 중 하나**(§6 표) 선택. 둘을 고르기 전에는 포탈이 "준비가 안 됐다"고 답한다.
- **포탈 앞 칸에 서서 포탈을 탭** → "1층으로 내려간다 / 아직" → `begin_run` → 1층. 종족·시작 장비는 Run 동안 바꿀 수 없다.
- NPC의 종족·무기도 같은 표에서 시드로 뽑는다(종족 3, 무기 10). 마법 무기를 든 NPC는 그 학파 첫 주문을 알고 준비한 채 나온다. 이 30명이 던전에서 층당 3~5명씩 나타난다(한 Run에 다 만나지 못한다).
- 화면 계약: `phase == "IDLE"`이 이 장면. 노드 `PlazaHud`(`HeroCard`, `SpeciesPick`, `KitPick`), `PortalPopup`(`Descend`). 결과 화면의 "새 Run"은 새 명부로 이 장면을 다시 연다. 아트는 기존 토큰·타일 재사용(새 그림 없음; 포탈은 계단 아이콘 계열의 코드 텍스처).
...
- 시작 장비는 광장 장면(§1.0)의 주인공 카드에서 고르는 **10개 중 하나**(숙련 10축에 하나씩). 방어구는 전원 `robe`.
