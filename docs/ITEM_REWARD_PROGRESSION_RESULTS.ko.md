...
- 종족 드롭 테이블을 `species-drops-v2`로 확장했다. goblin 사망 시 결정적으로 골드 12와 `FIREBOLT` 획득 행을 기록한다.
...
- 몬스터 이능은 획득과 장착을 분리했다. 보상 획득만으로 기존 loadout을 덮어쓰지 않는다.
...
- 이능 보상은 이벤트에 귀속되고 중복 ID를 한 번만 표시하지만, 현재 active skill의 기존 2-slot loadout을 보존한다. 계획서의 레벨별 최대 6칸 결속·제거 정책은 후속 범위다.
