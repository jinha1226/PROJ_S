...
- 종족 선택 직후 호출하던 마을 `START` 명령을 제거했다. 새 원정은 이미 생성된 던전 1층에서 시작한다. 마을 시스템 자체나 기존 저장 데이터의 위치는 변경하지 않는다.
- 모든 벽을 `wall_mid`로 그리던 매핑을 수정했다. 관측된 주변 바닥 방향에 따라 정면, 상단, 얇은 좌우 측면과 대각선 모서리 원본 타일을 선택한다.
- 미탐색 이웃의 지형은 타일 선택에 사용하지 않는다. 기억된 지형은 사용한다. 물리 충돌과 시야 판정의 한 칸 벽 크기는 유지한다.
- 기존 지형 투영 갱신에서만 선택하고 retained chunk의 기존 region 서명으로 변경을 감지한다. 프레임별 전체 맵 스캔이나 새로운 다시 그리기 타이머는 추가하지 않았다.
- 에셋 검증에 실제 종족 선택 → 던전 1층 진입, 벽 방향, 기억/미탐색 경계 검사를 추가했다.
