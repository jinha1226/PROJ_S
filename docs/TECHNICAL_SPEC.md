...
- 작업장 월드는 화면 크기와 분리된 1120×1040 논리 좌표를 사용한다.
- 최초 배율은 월드 전체가 보이는 overview fit으로 계산한다.
- 모바일은 두 손가락 pinch zoom과 확대 상태 한 손가락 drag를 지원한다.
- 화면의 `− / 배율 / +` 버튼은 pinch를 사용하기 어려운 환경을 위한 동일 기능이다.
- 데스크톱은 mouse wheel zoom과 drag를 지원한다.
- 화면 터치 좌표는 카메라 역변환 후 슬라임과 시설 hit test에 사용한다.
